...
Take this to a more general perspective, take \( n \) arbitrary solutions \( \{ \phi_1, \cdots, \phi_n \}\),
	...
	Consider a planar linear system \begin{equation}
	 x^\prime(t)=Ax(t), \quad A\in \mathbb{R}^{2\times 2} 
	 \label{eq: planar linear system}  
	\end{equation}
	we write \[
	 \tr(A)=T, \quad \det A= D
	\]
	\begin{definition}
		The equilibrium point \( 0 \) of the system \eqref{eq: planar linear system} is called \textbf{hyperbolic} if the matrix \( A \) satisfies that all eigenvalues \( \lambda_i \) satisfies that \[
		 \lambda_i\neq 0
		\]
	\end{definition}
	\begin{proposition}
	    A planar linear system is hyperbolic if and only if \[
			(D<0)\lor((D>0)\land(T\neq 0))
	    \]
	\end{proposition}
	\begin{definition}
	 A property \( P \) of matricies in \( \mathbb{R}^{n\times n}  \) is called \textbf{generic} if the set \[
		 S=\{A\in \mathbb{R}^{n\times n} : P(A)\}
	 \]
	 is open and dense in \( \mathbb{R}^{n\times n}  \).
	\end{definition}
\begin{definition}
    Two matrices \( A,B \) are conjugate to each other of there is an invertible matrix \( T \) such that \[
     B=T^{-1} A T
    \]
	In addition, in any group one says \( a \) is conjugate to \( b \) if \[
	 b=gag^{-1} 
	\]
	for some invertible element \( g \)
\end{definition}
Thus, since we know that the flow on the set of solutions to a differential equation is a group, we define \[
 \psi_t=h\circ \phi_t \circ h^{-1} 
\]
characterizing conjugacy. If \( h \) is a homeomorphism, then topological conjugacy holds. If \( h \) is a diffeomorphism, smooth conjugacy holds. 
\begin{definition}
 Let \( \phi(t) \) be a dynamical system on \( D\subset \mathbb{R}^n \), with \( t\geq 0 \) and \( a\in D \) a fixed point (equilibrium):
 \[
	 \phi_t(a)=(a)\quad \text{for all } t>\geq 0
 \]
 Then \( a \) is call stabvle if for every \( \varepsilon>0 \) there exist \( \delta>0 \) such that \[
  \norm{x-a} < \delta \implies \sup_{t\geq 0} \norm{\phi_t(x)-a} <\varepsilon
 \]
\end{definition}
\begin{definition}
    Respectively, \( a \) is unstable if it is not stable. 
\end{definition}
\begin{definition}
    \( a \) is asymtopically stable if: 
	\begin{enumerate}
		\item \( a \) is stable
		\item there exists \( \delta>0 \) such that 
			\[
				\norm{x-a} < \delta \implies \phi_t(x) \to a \text{ as } t\to \infty 
			\]
	\end{enumerate}
\end{definition}
\begin{theorem}
	...
	Let \[
	 x^\prime=f(x)
	\] where \( f\in C^1 \) and suppose \( a \) is a equilibrium:
	\[
	 f(a)=0
	\]
	Let \( A:DF(a) \). Assume \( A \) is hyperbolic. Let \( \Phi)t \) denote the nonlinear flow and \( \Psi_t=e^{tA}  \) the linearized flow. then there exists neighborhoods \( U  \) of \( a \) and \( V \) of \( 0 \) together with a homeomorphism \[
	 h:U\to V,
	\]
	such that \( g\circ \Phi_t=\Psi_t\circ h \) whenever both sides are defined and remain in the chose neighborhoods. Equivalently, \[
	 \Phi_t=h^{-1} \circ e^{tA} \circ h
	\]
\end{theorem}
\end{document}
