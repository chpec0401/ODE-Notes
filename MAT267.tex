...
Consider a inhomogeneous linear system:
\[x^\prime(t)=Ax(t)+f(t)\]
where we take \( A\in \mathbb{R}^{n\times n} \) and \( f \) is a time dependent function in \( \mathbb{R}^n \). Let \( y \) be a \textit{particular} solution to such a differential equation, then we know that,  by linearity \[
 x(t)=y(t)+e^{tA} v
\] is a solution for arbitary initial condition defining \( v \). Indeed \[
 x_1(t)=e^{At} v
\] is a solution to \( x^\prime(t)=Ax(t) \). Then 
\begin{align*}
	x^\prime(t)&=\left( y+x_1\right) ^\prime\\
			   &= y^\prime+x_1^\prime\\
			   &= A(y+x_1)+f(t)\\
			   &=Ax+f(t)
\end{align*}
Thus, the problem reduces to finding a particular solution to the original system. Of course, we know very well the solution to the linear system using an integration factor \( e^{At}\):
\begin{align*}
	e^{At} y^\prime +e^{At} Ay&=e^{At} f(t)\\
	(e^{At} y)^\prime &= e^{At} f(t)\\
	e^{At} y&= y_0 +\int_{t_0}^{t} e^{As} f(s）
\end{align*}
